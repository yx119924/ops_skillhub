...
✅ 推荐（按职能维度）：
- `01-troubleshooting` —— 故障排查（incident triage、网络诊断、性能问题等）
- `02-deployment` —— 部署方案（compose、k8s、脚本、回滚等）
- `03-sre` —— SRE 建设（SOP、容量、架构、稳定性等）
- `04-basics` —— 基础运维（磁盘、日志、配置、用户管理等）

新分类时，先确认是否能归到现有 4 类之一。如果确实需要新增分类（如 `05-security`），在现有分类后面追加两位数字。

判断分类归属的快速决策树：

```
是新分类的 skill 吗？
├─ 是被动响应告警/异常 → 01-troubleshooting
├─ 是部署/变更/发布相关 → 02-deployment
├─ 是工程化能力/SOP/规范 → 03-sre
├─ 是日常运维高频操作 → 04-basics
└─ 以上都不是 → 与我讨论新分类
```
